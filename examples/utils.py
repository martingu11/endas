...
def make_data(model_fn, x0, dt, H, Q, R, nsteps, nspin=None):
    """
    Generates true state of a system and synthetic observations for given number of steps.

    The true state is generated by propagating initial state with a model. At each time step, observations of the system
    are generated from the true state by adding random noise.

    Args:
        model_fn     : Evolution model callable object for propagating the model state forward in time.
    ...
    if nspin is not None and nspin > 0:
        for i in range(1, nspin):
            model_fn(xt, dt)
            if Q is not None:
                xt += Q.random_multivariate_normal()
    ...
    for i in range(nsteps):
        xtrue[:, i] = xt
        zt = H.dot(xt)
        zobs[:, i] = zt + R.random_multivariate_normal()
