        ...
        # Localized analysis
            ...
            if m > 0:
                ...
                for di in range(self._num_domains):
                    ...
                    local_zindexes, local_zdist = ls.ssp.get_local_observations(di, z_coords, self._loc_strategy.taper_fn)
                    ...
                    if local_m > 0:
                        ...
                        if executor is not None:
                            ...
                            if local_X5s == None: local_X5s = local_X5
            ...
            for di in range(self._num_domains):
                local_start, local_n = self._loc_statesize_limits[di]
                self.localization_strategy.put_state(di,
                                                     self._Aa_enkf[local_start:local_start + local_n, :],
                                                     self._Af)
        ...
        for j in range(k-1, k-1-self._lag, -1):
            ...
            # N is the ensemble size. Likewise, X5 is an DxNxN array.
                ...
                for di in range(self._num_domains):
                    ...
                    if self._haveX5[di]: # and self._last_analysis_j[d_i] <= j:
                        X5d = self._X5[di, :, :]
                        if self.forgetting_factor != 1.0:
                            X4 = X5d - eyeN
                            X4 *= self.forgetting_factor
                    ...
                    if Aj_is_result:
                        self.localization_strategy.put_state(di, local_Aj, As)
        ...
        for j in range(k - 1, k - 1 - self._lag, -1):
            ...
            if self._num_domains == 0:
                ...
                for di in range(self._num_domains):
                    ...
                    local_Aj = Aj[local_start:local_start + local_n, :]
                    assert local_Aj.shape == (local_n, N)
                    self.localization_strategy.put_state(di, local_Aj, As_buffer)
                As = As_buffer
